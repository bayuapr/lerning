...
class tabel_pengajar extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('pengajar_model', 'login');
    ...
    public function ajax_list()
    {
        ...
        foreach ($list as $pengajar) {
            ...
            $row = array();
            $row[] = $no;
            $row[] = $pengajar->username;
            $row[] = $pengajar->nama;
            $row[] = $pengajar->alamat;

            //add html for action
            $row[] = '<a class="btn btn-sm btn-primary" href="javascript:void(0)" title="Edit" onclick="edit_pengajar(' . "'" . $pengajar->id_pengajar . "'" . ')"><i class="glyphicon glyphicon-pencil"></i> Edit</a>
				  <a class="btn btn-sm btn-danger" href="javascript:void(0)" title="Hapus" onclick="delete_pengajar(' . "'" . $pengajar->id_pengajar . "'" . ')"><i class="glyphicon glyphicon-trash"></i> Delete</a>';
    ...
    public function ajax_update()
    {
        ...
        $this->login->update(array('id_pengajar' => $this->input->post('id_pengajar')), $data);
        echo json_encode(array("status" => TRUE));
    }
        ...
        if (isset($_FILES["file"]["name"])) {
            ...
            foreach ($object->getWorksheetIterator() as $worksheet) {
                ...
                for ($row = 2; $row <= $highestRow; $row++) {
                    $id_pengajar = $worksheet->getCellByColumnAndRow(0, $row)->getValue();
